...
const ProductsPage = () => {
  ...
  const [loadingMap, setLoadingMap] = useState({}); // tracks per-product loading state
  ...
  const handleAddToCart = async (e, product) => {
    ...
    if (isGuest) {
      ...
      try {
        ...
        // Notify navbar to re-fetch cart count from DB
...
const ProductCard = ({ product, added, isLoading, onCardClick, onAddToCart }) => {
  ...
  const getButtonBg = () => {
    ...
    if (isLoading) return "#f97316"; // orange while loading
    if (added) return "#16a34a";     // green when added
